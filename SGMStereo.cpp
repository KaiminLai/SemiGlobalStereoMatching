	...
	const int aggregationWindowRadius)
{
	...
	if (censusWindowRadius < 1 || censusWindowRadius > 4) {
		throw std::invalid_argument("[SGMStereo::setDataCostParameters] window radius of Census transform must be from 1 to 4");
	}
...
void SGMStereo::compute(Mat& leftImage, Mat& rightImage, Mat& disparityImage)
{
	...
	for (int y = 0; y < height_; ++y) {
		float* lDisData = (float*)disparityImage.ptr<float>(y);
...
void SGMStereo::initialize(const Mat& leftImage, const Mat& rightImage) {
	setImageSize(leftImage, rightImage);
	allocateDataBuffer();
}

void SGMStereo::setImageSize(const Mat& leftImage, const Mat& rightImage) {
...
void SGMStereo::computeCostImage(Mat& leftImage, Mat& rightImage) {
...
void SGMStereo::convertToGrayscale( Mat& leftImage,
	 Mat& rightImage,
	unsigned char* leftGrayscaleImage,
	unsigned char* rightGrayscaleImage) 
{
	cvtColor(leftImage, leftImage, CV_BGR2RGB);
	cvtColor(rightImage, rightImage, CV_BGR2RGB);
	leftImage.convertTo(leftImage, CV_64F, 1 / 255.0f);
	rightImage.convertTo(rightImage, CV_64F, 1 / 255.0f);
	Mat tmp;

	leftImage.convertTo(tmp, CV_32F);
	cvtColor(tmp, lGray, CV_RGB2GRAY);
	lGray.convertTo(lGray, CV_8U, 255);

	rightImage.convertTo(tmp, CV_32F);
	...
	for (int y = 0; y < height_; y++){
		...
		for (int x = 0; x < width_; x++){
			leftGrayscaleImage[width_*y + x] = static_cast<unsigned char>(pLData[x]);
			rightGrayscaleImage[width_*y + x] = static_cast<unsigned char>(pRData[x]);
		}
	}
}
...
void SGMStereo::computeLeftCostImage(const unsigned char* leftGrayscaleImage, const unsigned char* rightGrayscaleImage) {
	...
	lCode = new std::bitset<CENSUS_BIT>[width_ * height_];
	rCode = new std::bitset<CENSUS_BIT>[width_ * height_];
	computeCensusImage();
	//computeCensusImage(rGray, rCode);
	...
	std::bitset<CENSUS_BIT>* leftCensusRow = lCode;
	std::bitset<CENSUS_BIT>* rightCensusRow = rCode;
...
void SGMStereo::computeCensusImage() {
	std::bitset<CENSUS_BIT>* pLCode = lCode;
	std::bitset<CENSUS_BIT>* pRCode = rCode;
	for (int y = 0; y < height_; y++){
		uchar* pLData = (uchar*)(lGray.ptr<uchar>(y));
		uchar* pRData = (uchar*)(rGray.ptr<uchar>(y));
		for (int x = 0; x < width_; x++){
			int bitCnt = 0;
			for (int wy = -censusWindowRadius_; wy <= censusWindowRadius_; wy++) {
				int qy = (y + wy + height_) % height_;
				uchar* qLData = (uchar*)(lGray.ptr<uchar>(qy));
				uchar* qRData = (uchar*)(rGray.ptr<uchar>(qy));
				for (int wx = -censusWindowRadius_; wx <= censusWindowRadius_; wx++) {
					if (wy != 0 || wx != 0) {
						int qx = (x + wx + width_) % width_;
...
void SGMStereo::calcTopRowCost(unsigned char*& leftSobelRow, std::bitset<CENSUS_BIT>*& leftCensusRow,
...
void SGMStereo::calcRowCosts(unsigned char*& leftSobelRow, std::bitset<CENSUS_BIT>*& leftCensusRow,
	unsigned char*& rightSobelRow, std::bitset<CENSUS_BIT>*& rightCensusRow,
	unsigned short* costImageRow)
{
...
void SGMStereo::addPixelwiseHamming(const std::bitset<CENSUS_BIT>* leftCensusRow, const std::bitset<CENSUS_BIT>* rightCensusRow) {
	for (int x = 0; x < disparityTotal_; ++x) {
		std::bitset<CENSUS_BIT> leftCencusCode = leftCensusRow[x];
		int hammingDistance = 0;
		for (int d = 0; d <= x; ++d) {
			std::bitset<CENSUS_BIT> rightCensusCode = rightCensusRow[x - d];
			//hammingDistance = static_cast<int>(_mm_popcnt_u32(static_cast<unsigned int>((leftCencusCode^rightCensusCode).count())));
			hammingDistance = (leftCencusCode^rightCensusCode).count();
			//pixelwiseCostRow_[disparityTotal_*x + d] += static_cast<unsigned char>(hammingDistance*censusWeightFactor_);
			pixelwiseCostRow_[disparityTotal_*x + d] = static_cast<unsigned char>(hammingDistance);
		}
		//hammingDistance = static_cast<unsigned char>(hammingDistance*censusWeightFactor_);
		hammingDistance = static_cast<unsigned char>(CENSUS_BIT);
		for (int d = x + 1; d < disparityTotal_; ++d) {
	...
	for (int x = disparityTotal_; x < width_; ++x) {
		...
		for (int d = 0; d < disparityTotal_; ++d) {
			std::bitset<CENSUS_BIT> rightCensusCode = rightCensusRow[x - d];
			//int hammingDistance = static_cast<int>(_mm_popcnt_u32(static_cast<unsigned int>((leftCencusCode^rightCensusCode).count())));
			int hammingDistance = (leftCencusCode^rightCensusCode).count();
			//pixelwiseCostRow_[disparityTotal_*x + d] += static_cast<unsigned char>(hammingDistance*censusWeightFactor_);
			pixelwiseCostRow_[disparityTotal_*x + d] = static_cast<unsigned char>(hammingDistance);
		}
	}
